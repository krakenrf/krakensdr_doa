...
def read_config_file_dict(config_fname=daq_config_filename):
    ...
    ini_data["en_noise_source_ctr"] = parser.getint(
        "daq", "en_noise_source_ctr")
    ini_data["cpi_size"] = parser.getint("pre_processing", "cpi_size")
    ini_data["decimation_ratio"] = parser.getint(
        "pre_processing", "decimation_ratio")
    ini_data["fir_relative_bandwidth"] = parser.getfloat(
        "pre_processing", "fir_relative_bandwidth")
    ini_data["fir_tap_size"] = parser.getint("pre_processing", "fir_tap_size")
    ini_data["fir_window"] = parser.get("pre_processing", "fir_window")
    ini_data["en_filter_reset"] = parser.getint(
        "pre_processing", "en_filter_reset")
    ...
    ini_data["gain_lock_interval"] = parser.getint(
        "calibration", "gain_lock_interval")
    ini_data["require_track_lock_intervention"] = parser.getint(
        "calibration", "require_track_lock_intervention")
    ini_data["cal_track_mode"] = parser.getint("calibration", "cal_track_mode")
    ini_data["amplitude_cal_mode"] = parser.get(
        "calibration", "amplitude_cal_mode")
    ini_data["cal_frame_interval"] = parser.getint(
        "calibration", "cal_frame_interval")
    ini_data["cal_frame_burst_size"] = parser.getint(
        "calibration", "cal_frame_burst_size")
    ini_data["amplitude_tolerance"] = parser.getint(
        "calibration", "amplitude_tolerance")
    ini_data["phase_tolerance"] = parser.getint(
        "calibration", "phase_tolerance")
    ini_data["maximum_sync_fails"] = parser.getint(
        "calibration", "maximum_sync_fails")
    ini_data["iq_adjust_source"] = parser.get(
        "calibration", "iq_adjust_source")
    ini_data["iq_adjust_amplitude"] = parser.get(
        "calibration", "iq_adjust_amplitude")
    ini_data["iq_adjust_time_delay_ns"] = parser.get(
        "calibration", "iq_adjust_time_delay_ns")

    ini_data["adpis_gains_init"] = parser.get("adpis", "adpis_gains_init")

    ini_data["out_data_iface_type"] = parser.get(
        "data_interface", "out_data_iface_type")
...
def set_clicked(web_interface, clickData):
    ...
    if curveNumber >= M:
        ...
        web_interface.module_signal_processor.vfo_freq[idx] = int(
    ...
    try:
        ...
        for data_entry in que_data_packet:
            if data_entry[0] == "iq_header":
                ...
                web_interface.daq_dsp_latency = data_entry[1] + \
                ...
                web_interface.logger.warning(
                    "Unknown data entry: {:s}".format(
                        data_entry[0]))
    except queue.Empty:
        # Handle empty queue here
...
def fetch_gps_data(app, web_interface):
    ...
    web_interface.gps_timer = Timer(
        1, fetch_gps_data, args=(
            app, web_interface))
    web_interface.gps_timer.start()


def settings_change_watcher(
        web_interface, settings_file_path, last_attempt_failed=False):
        ...
        if time_delta > 0:
            ...
            try:
                ...
                if not last_attempt_failed:
                    web_interface.logger.error(
                        "Problem loading settings file: %s", ex)
                ...
                web_interface.ant_spacing_meters = float(
                    dsp_settings.get("ant_spacing_meters", 0.5))

                web_interface.en_system_control = [1] if dsp_settings.get(
                    "en_system_control", False) else []
                web_interface.en_beta_features = [1] if dsp_settings.get(
                    "en_beta_features", False) else []

                web_interface.module_signal_processor.en_DOA_estimation = dsp_settings.get(
                    "en_doa", 0)
                web_interface.module_signal_processor.DOA_decorrelation_method = dsp_settings.get(
                    "doa_decorrelation_method", 0
                )

                web_interface.module_signal_processor.DOA_ant_alignment = dsp_settings.get(
                    "ant_arrangement", "ULA")
                web_interface.ant_spacing_meters = float(
                    dsp_settings.get("ant_spacing_meters", 0.5))
                ...
                # Station Information
                web_interface.module_signal_processor.station_id = dsp_settings.get(
                    "station_id", "NO-CALL")
                web_interface.location_source = dsp_settings.get(
                    "location_source", "None")
                web_interface.module_signal_processor.latitude = dsp_settings.get(
                    "latitude", 0.0)
                web_interface.module_signal_processor.longitude = dsp_settings.get(
                    "longitude", 0.0)
                web_interface.module_signal_processor.heading = dsp_settings.get(
                    "heading", 0.0)
                web_interface.module_signal_processor.krakenpro_key = dsp_settings.get(
                    "krakenpro_key", 0.0)
                ...
                web_interface.module_signal_processor.vfo_mode = dsp_settings.get(
                    "vfo_mode", "Standard")
                web_interface.module_signal_processor.vfo_default_squelch_mode = dsp_settings.get(
                    "vfo_default_squelch_mode", "Auto"
                )
                web_interface.module_signal_processor.vfo_default_demod = dsp_settings.get(
                    "vfo_default_demod", "None")
                web_interface.module_signal_processor.vfo_default_iq = dsp_settings.get(
                    "vfo_default_iq", "False")
                web_interface.module_signal_processor.max_demod_timeout = int(
                    dsp_settings.get("max_demod_timeout", 60))
                web_interface.module_signal_processor.dsp_decimation = int(
                    dsp_settings.get("dsp_decimation", 0))
                web_interface.module_signal_processor.active_vfos = int(
                    dsp_settings.get("active_vfos", 0))
                web_interface.module_signal_processor.output_vfo = int(
                    dsp_settings.get("output_vfo", 0))
                web_interface.compass_offset = dsp_settings.get(
                    "compass_offset", 0)
                ...
                web_interface.module_signal_processor.en_peak_hold = dsp_settings.get(
                    "en_peak_hold", 0)
                ...
                for i in range(web_interface.module_signal_processor.max_vfos):
                    ...
                    web_interface.module_signal_processor.vfo_fir_order_factor[i] = int(
                        dsp_settings.get(
                            ...
                            DEFAULT_VFO_FIR_ORDER_FACTOR)
                    )
                    web_interface.module_signal_processor.vfo_freq[i] = float(
                    ...
                    web_interface.module_signal_processor.vfo_iq[i] = dsp_settings.get(
                        "vfo_iq_" + str(i), "Default")

                web_interface.module_signal_processor.DOA_algorithm = dsp_settings.get(
                    "doa_method", "MUSIC")
                web_interface.module_signal_processor.DOA_expected_num_of_sources = dsp_settings.get(
                    "expected_num_of_sources", 1
                )
                web_interface._doa_fig_type = dsp_settings.get(
                    "doa_fig_type", "Linear")
                web_interface.module_signal_processor.doa_measure = web_interface._doa_fig_type
                web_interface.module_signal_processor.ula_direction = dsp_settings.get(
                    "ula_direction", "Both")
                web_interface.module_signal_processor.array_offset = int(
                ...
                if abs(freq_delta) > 0.001 or abs(gain_delta) > 0.001:
                    ...
                    for i in range(
                            web_interface.module_signal_processor.max_vfos):
                        half_band_width = (
                            web_interface.module_signal_processor.vfo_bw[i] / 10**6) / 2
                        min_freq = web_interface.daq_center_freq - \
                            web_interface.daq_fs / 2 + half_band_width
                        max_freq = web_interface.daq_center_freq + \
                            web_interface.daq_fs / 2 - half_band_width
                        if (min_freq > (web_interface.module_signal_processor.vfo_freq[i] / 10**6) or max_freq < (
                            ...
                            web_interface.module_signal_processor.vfo_freq[
                                i] = web_interface.module_receiver.daq_center_freq
...
def update_daq_status(app, web_interface):
    ...
    if web_interface.daq_update_rate < 1:
        daq_update_rate_str = "{:d} ms".format(
            round(web_interface.daq_update_rate * 1000))
    else:
