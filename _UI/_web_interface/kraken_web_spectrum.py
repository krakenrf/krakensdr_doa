...
def init_spectrum_fig(webInterface_inst, fig_layout, trace_colors):
    ...
    for m in range(
            0, webInterface_inst.module_receiver.M):  # +1 for the auto decimation window selection
...
def plot_spectrum(app, webInterface_inst, spectrum_fig, waterfall_fig):
    ...
    if webInterface_inst.reset_spectrum_graph_flag:
        ...
        x = webInterface_inst.spectrum[0, :] + \
            webInterface_inst.daq_center_freq * 10 ** 6
        ...
        if webInterface_inst.module_signal_processor.spectrum_fig_type == 'Single':
            ...
            if hide_channels_from > 1:
                ...
                peak_hold_color_rgb = px.colors.label_rgb(
                    px.colors.hex_to_rgb(spectrum_fig.data[0]['line']['color']))
        ...
        for i in range(webInterface_inst.module_signal_processor.max_vfos):
            if i < webInterface_inst.module_signal_processor.active_vfos:
                spectrum_fig.data[webInterface_inst.module_receiver.M +
                                  (i * 2)]['visible'] = True
                spectrum_fig.data[webInterface_inst.module_receiver.M +
                                  (i * 2 + 1)]['visible'] = True
        ...
        waterfall_fig.update_xaxes(
            tickfont_size=1, range=[
                np.min(x), np.max(x)], showgrid=False)
        ...
        for i in range(webInterface_inst.module_signal_processor.active_vfos):
            ...
            maxIndex = webInterface_inst.spectrum[webInterface_inst.module_receiver.M + (
                i * 2 + 1), :].argmax()

            reverseSpectrum = webInterface_inst.spectrum[webInterface_inst.module_receiver.M + (
                i * 2 + 1), ::-1]
