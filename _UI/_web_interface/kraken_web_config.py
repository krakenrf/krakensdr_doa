...
def write_config_file_dict(webInterface_inst, param_dict, dsp_settings):
    ...
    parser['daq']['en_noise_source_ctr'] = str(
        param_dict['en_noise_source_ctr'])
    # Set these for reconfigure
    parser['daq']['center_freq'] = str(
        int(webInterface_inst.module_receiver.daq_center_freq))
    parser['pre_processing']['cpi_size'] = str(param_dict['cpi_size'])
    parser['pre_processing']['decimation_ratio'] = str(
        param_dict['decimation_ratio'])
    parser['pre_processing']['fir_relative_bandwidth'] = str(
        param_dict['fir_relative_bandwidth'])
    parser['pre_processing']['fir_tap_size'] = str(param_dict['fir_tap_size'])
    parser['pre_processing']['fir_window'] = str(param_dict['fir_window'])
    parser['pre_processing']['en_filter_reset'] = str(
        param_dict['en_filter_reset'])
    ...
    parser['calibration']['gain_lock_interval'] = str(
        param_dict['gain_lock_interval'])
    parser['calibration']['require_track_lock_intervention'] = str(
        param_dict['require_track_lock_intervention'])
    parser['calibration']['cal_track_mode'] = str(param_dict['cal_track_mode'])
    parser['calibration']['amplitude_cal_mode'] = str(
        param_dict['amplitude_cal_mode'])
    parser['calibration']['cal_frame_interval'] = str(
        param_dict['cal_frame_interval'])
    parser['calibration']['cal_frame_burst_size'] = str(
        param_dict['cal_frame_burst_size'])
    parser['calibration']['amplitude_tolerance'] = str(
        param_dict['amplitude_tolerance'])
    parser['calibration']['phase_tolerance'] = str(
        param_dict['phase_tolerance'])
    parser['calibration']['maximum_sync_fails'] = str(
        param_dict['maximum_sync_fails'])
    parser['calibration']['iq_adjust_source'] = str(
        param_dict['iq_adjust_source'])
    parser['calibration']['iq_adjust_amplitude'] = str(
        param_dict['iq_adjust_amplitude'])
    parser['calibration']['iq_adjust_time_delay_ns'] = str(
        param_dict['iq_adjust_time_delay_ns'])
    parser['adpis']['adpis_gains_init'] = str(param_dict['adpis_gains_init'])

    ini_parameters = parser._sections

    error_list = ini_checker.check_ini(
        ini_parameters, dsp_settings.get(
            "en_hw_check", 0))  # settings.en_hw_check)
...
def generate_config_page_layout(webInterface_inst):
    ...
    en_data_record = [
        1] if webInterface_inst.module_signal_processor.en_data_record else []
    en_doa_values = [
        1] if webInterface_inst.module_signal_processor.en_DOA_estimation else []

    en_optimize_short_bursts = [
        1] if webInterface_inst.module_signal_processor.optimize_short_bursts else []
    en_peak_hold = [
        1] if webInterface_inst.module_signal_processor.en_peak_hold else []

    en_fixed_heading = [
        1] if webInterface_inst.module_signal_processor.fixed_heading else []
    ...
    start_stop_card = \
        html.Div([
            html.Div([html.Div(
                [html.Button('Start Processing', id='btn-start_proc',
                             className="btn_start", n_clicks=0)],
                className="ctr_toolbar_item"),
                html.Div([html.Button('Stop Processing', id='btn-stop_proc', className="btn_stop", n_clicks=0)],
                         className="ctr_toolbar_item"),
                html.Div(
    ...
    daq_config_card_list = \
        [
            ...
            html.Div([
                ...
                dcc.Input(id='daq_center_freq', value=webInterface_inst.module_receiver.daq_center_freq / 10 ** 6,
                          type='number', debounce=True, className="field-body-textbox")
            ], className="field"),
            html.Div([
                ...
                dcc.Dropdown(id='daq_rx_gain',
                             ...
                             value=webInterface_inst.module_receiver.daq_rx_gain, clearable=False,
            ...
            html.Div([html.Div("Basic DAQ Configuration", id="label_en_basic_daq_cfg", className="field-label"),
                      dcc.Checklist(options=option, id="en_basic_daq_cfg", className="field-body",
            ...
            html.Div([
                ...
                html.Div([
                    html.Div("Active Configuration: " + webInterface_inst.active_daq_ini_cfg, id="active_daq_ini_cfg",
                             className="field-label"),
                ], className="field"),
                html.Div([
                    html.Div(webInterface_inst.daq_cfg_ini_error, id="daq_ini_check", className="field-label",
                             style={"color": "#e74c3c"}),
                ], className="field"),
                ...
                html.Div([
                    html.Div(
                        ...
                        className="field-label"),
                    dcc.Input(id='cfg_data_block_len', value=cfg_data_block_len, type='number', debounce=True,
                ...
                html.Div([
                    html.Div(
                        ...
                        className="field-label"),
                    dcc.Input(id='cfg_recal_interval', value=cfg_recal_interval, type='number', debounce=True,
                              className="field-body-textbox")
                ], className="field"),

                html.Div([html.Div("Advanced Custom DAQ Configuration", id="label_en_advanced_daq_cfg",
                                   className="field-label"),
                          dcc.Checklist(options=option, id="en_advanced_daq_cfg", className="field-body",
                ...
                html.Div([
                    ...
                    html.Div([
                        ...
                        dcc.Input(id='cfg_rx_channels', value=daq_cfg_dict['num_ch'], type='number', debounce=True,
                                  className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div("Bias Tee Control:", className="field-label"),
                        dcc.Input(id='cfg_en_bias_tee', value=daq_cfg_dict['en_bias_tee'], type='text', debounce=True,
                                  className="field-body-textbox")
                    ], className="field"),
                    ...
                    html.Div([
                        ...
                        dcc.Dropdown(id='cfg_daq_buffer_size',
                                     ...
                                     value=daq_cfg_dict['daq_buffer_size'], style={
                    ...
                    html.Div([
                        ...
                        dcc.Dropdown(id='cfg_sample_rate',
                                     ...
                                     value=daq_cfg_dict['sample_rate'] / 10 ** 6, style={"display": "inline-block"},
                                     className="field-body")
                    ], className="field"),
                    html.Div([
                        html.Div("Enable Noise Source Control:", className="field-label",
                                 id="label_en_noise_source_ctr"),
                        dcc.Checklist(options=option, id="en_noise_source_ctr", className="field-body",
                    ...
                    html.Div([
                        html.Div(
                            ...
                            className="field-label",
                            id="label_cpi_size"),
                        dcc.Input(id='cfg_cpi_size', value=daq_cfg_dict['cpi_size'], type='number', debounce=True,
                                  className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                        ...
                        dcc.Input(id='cfg_decimation_ratio', value=daq_cfg_dict['decimation_ratio'], type='number',
                                  debounce=True, className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                        ...
                        dcc.Input(id='cfg_fir_bw', value=daq_cfg_dict['fir_relative_bandwidth'], type='number',
                                  debounce=True, className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                        ...
                        dcc.Input(id='cfg_fir_tap_size', value=daq_cfg_dict['fir_tap_size'], type='number',
                                  debounce=True, className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                        ...
                        dcc.Dropdown(id='cfg_fir_window',
                                     ...
                                     value=daq_cfg_dict['fir_window'], style={
                                         "display": "inline-block"},
                                     className="field-body")
                    ], className="field"),
                    html.Div([
                        ...
                        dcc.Checklist(options=option, id="en_filter_reset", className="field-body",
                    ...
                    html.Div([
                        ...
                        dcc.Input(id='cfg_corr_size', value=daq_cfg_dict['corr_size'], type='number', debounce=True,
                                  className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                        ...
                        dcc.Input(id='cfg_std_ch_ind', value=daq_cfg_dict['std_ch_ind'], type='number', debounce=True,
                                  className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                    ...
                    html.Div([
                        ...
                        dcc.Input(id='cfg_gain_lock', value=daq_cfg_dict['gain_lock_interval'], type='number',
                                  debounce=True, className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div("Require Track Lock Intervention (For Kerberos):", className="field-label",
                                 id="label_require_track_lock"),
                        dcc.Checklist(options=option, id="en_req_track_lock_intervention", className="field-body",
                    ...
                    html.Div([
                        ...
                        dcc.Dropdown(id='cfg_cal_track_mode',
                                     ...
                                     value=daq_cfg_dict['cal_track_mode'], style={
                                         "display": "inline-block"},
                                     className="field-body"),
                    ], className="field"),
                    html.Div([
                        ...
                        dcc.Dropdown(id='cfg_amplitude_cal_mode',
                                     ...
                                     value=daq_cfg_dict['amplitude_cal_mode'], style={
                                         "display": "inline-block"},
                                     className="field-body"),
                    ], className="field"),
                    html.Div([
                        ...
                        dcc.Input(id='cfg_cal_frame_interval', value=daq_cfg_dict['cal_frame_interval'], type='number',
                                  debounce=True, className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div("Calibration Frame Burst Size:", className="field-label",
                                 id="label_calibration_frame_burst_size"),
                        dcc.Input(id='cfg_cal_frame_burst_size', value=daq_cfg_dict['cal_frame_burst_size'],
                                  type='number', debounce=True, className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                        ...
                        dcc.Input(id='cfg_amplitude_tolerance', value=daq_cfg_dict['amplitude_tolerance'],
                                  type='number', debounce=True, className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                        ...
                        dcc.Input(id='cfg_phase_tolerance', value=daq_cfg_dict['phase_tolerance'], type='number',
                                  debounce=True, className="field-body-textbox")
                    ], className="field"),
                    html.Div([
                        html.Div(
                        ...
                        dcc.Input(id='cfg_max_sync_fails', value=daq_cfg_dict['maximum_sync_fails'], type='number',
                                  debounce=True, className="field-body-textbox")
                    ], className="field"),
                    ...
                    html.Div([
                        ...
                        dcc.Dropdown(id='cfg_iq_adjust_source',
                                     ...
                                     value=daq_cfg_dict['iq_adjust_source'], style={
                                         "display": "inline-block"},
                                     className="field-body"),
                    ], className="field"),
                    ...
                    html.Div([
                        ...
                        dcc.Input(id='cfg_iq_adjust_amplitude', value=daq_cfg_dict['iq_adjust_amplitude'], type='text',
                                  debounce=True, className="field-body-textbox")
                    ], className="field"),

                    html.Div([
                        html.Div("IQ Adjust Time Delay (ns) :", className="field-label",
                                 id="label_iq_adjust_time_delay_ns"),
                        dcc.Input(id='cfg_iq_adjust_time_delay_ns', value=daq_cfg_dict['iq_adjust_time_delay_ns'],
                                  type='text', debounce=True, className="field-body-textbox")
                    ], className="field"),
    ...
    daq_status_card = \
        html.Div([
            ...
            html.Div([html.Div("Update Rate:", id="label_daq_update_rate", className="field-label"),
                      html.Div("- ms", id="body_daq_update_rate", className="field-body")], className="field"),
            html.Div([html.Div("Latency:", id="label_daq_dsp_latency", className="field-label"),
                      html.Div("- ms", id="body_daq_dsp_latency", className="field-body")], className="field"),
            html.Div([html.Div("Frame Index:", id="label_daq_frame_index", className="field-label"),
                      html.Div("-", id="body_daq_frame_index", className="field-body")], className="field"),
            html.Div([html.Div("Frame Type:", id="label_daq_frame_type", className="field-label"),
                      html.Div("-", id="body_daq_frame_type", className="field-body")], className="field"),
            html.Div([html.Div("Frame Sync:", id="label_daq_frame_sync", className="field-label"),
                      html.Div("LOSS", id="body_daq_frame_sync", className="field-body", style={"color": "#e74c3c"})],
                     className="field"),
            html.Div([html.Div("Power Level:", id="label_daq_power_level", className="field-label"),
                      html.Div("-", id="body_daq_power_level", className="field-body")], className="field"),
            html.Div([html.Div("Connection Status:", id="label_daq_conn_status", className="field-label"),
                      html.Div("Disconnected", id="body_daq_conn_status", className="field-body",
                               style={"color": "#e74c3c"})], className="field"),
            html.Div([html.Div("Sample Delay Sync:", id="label_daq_delay_sync", className="field-label"),
                      html.Div("LOSS", id="body_daq_delay_sync", className="field-body", style={"color": "#e74c3c"})],
                     className="field"),
            html.Div([html.Div("IQ Sync:", id="label_daq_iq_sync", className="field-label"),
                      html.Div("LOSS", id="body_daq_iq_sync", className="field-body", style={"color": "#e74c3c"})],
                     className="field"),
            html.Div([html.Div("Noise Source State:", id="label_daq_noise_source", className="field-label"),
                      html.Div("Disabled", id="body_daq_noise_source", className="field-body",
                               style={"color": "#7ccc63"})], className="field"),
            html.Div([html.Div("Center Frequecy [MHz]:", id="label_daq_rf_center_freq", className="field-label"),
                      html.Div("- MHz", id="body_daq_rf_center_freq", className="field-body")], className="field"),
            html.Div([html.Div("Sampling Frequency [MHz]:", id="label_daq_sampling_freq", className="field-label"),
                      html.Div("- MHz", id="body_daq_sampling_freq", className="field-body")], className="field"),
            html.Div([html.Div("DSP Decimated BW [MHz]:", id="label_dsp_decimated_bw", className="field-label"),
                      html.Div("- MHz", id="body_dsp_decimated_bw", className="field-body")], className="field"),
            html.Div([html.Div("VFO Range [MHz]:", id="label_vfo_range", className="field-label"),
                      html.Div("- MHz", id="body_vfo_range", className="field-body")], className="field"),
            html.Div([html.Div("Data Block Length [ms]:", id="label_daq_cpi", className="field-label"),
                      html.Div("- ms", id="body_daq_cpi", className="field-body")], className="field"),
            html.Div([html.Div("RF Gains [dB]:", id="label_daq_if_gain", className="field-label"),
                      html.Div("[,] dB", id="body_daq_if_gain", className="field-body")], className="field"),
            html.Div([html.Div("VFO-0 Power [dB]:", id="label_max_amp", className="field-label"),
                      html.Div("-", id="body_max_amp", className="field-body")], className="field"),
        ], className="card")
        ...
        html.Div([
            ...
            html.Div([html.Span("Array Configuration: ", id="label_ant_arrangement", className="field-label"),
                      dcc.RadioItems(
                          ...
                          ], value=webInterface_inst.module_signal_processor.DOA_ant_alignment, className="field-body",
                          labelStyle={
                              'display': 'inline-block',
            ...
            html.Div([
                ...
                dcc.Input(id='custom_array_x_meters',
                          value=','.join(
                              ['%.2f' % num for num in webInterface_inst.custom_array_x_meters]),
                          type='text', debounce=True, className="field-body-textbox")
            ], id="customx", className="field"),

            html.Div([
                html.Div("Custom Y [m]:", className="field-label"),
                dcc.Input(id='custom_array_y_meters',
                          value=','.join(
                              ['%.2f' % num for num in webInterface_inst.custom_array_y_meters]),
                          type='text', debounce=True, className="field-body-textbox")
            ], id="customy", className="field"),

            html.Div([
                html.Div([html.Div("[meter]:", id="label_ant_spacing_meter", className="field-label"),
                          dcc.Input(id="ant_spacing_meter", value=ant_spacing_meter, type='number', debounce=True,
                                    className="field-body-textbox")]),
                html.Div(
                    [html.Div("Wavelength Multiplier:", id="label_ant_spacing_wavelength", className="field-label"),
                     html.Div("1", id="body_ant_spacing_wavelength", className="field-body")], className="field"),
            ], id="antspacing", className="field"),

            html.Div([html.Div("", id="ambiguity_warning",
                     className="field", style={"color": "#f39c12"})]),
            ...
            html.Div([html.Div("", id="uca_decorrelation_warning",
                     className="field", style={"color": "#f39c12"})]),
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.Dropdown(id='doa_fig_type',
                             ...
                             value=webInterface_inst._doa_fig_type, style={
                                 "display": "inline-block"},
                             className="field-body"),
            ], className="field"),
            ...
            html.Div([
                ...
                dcc.Input(id="compass_offset", value=webInterface_inst.compass_offset, type='number', debounce=True,
                          className="field-body-textbox"),
            ], className="field"),
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.Dropdown(id='doa_format_type',
                             options=[
                                 ...
                                 {'label': 'Kraken Pro Local',
                                     'value': 'Kraken Pro Local'},
            ...
            html.Div([
                ...
                dcc.Dropdown(id='loc_src_dropdown',
                             ...
                             value=webInterface_inst.location_source, style={
                                 "display": "inline-block"},
                             className="field-body"),
            ], className="field"),
            html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Div(
                    ...
                    className="field-label"),
                dcc.Input(id='dsp_decimation', value=webInterface_inst.module_signal_processor.dsp_decimation,
                          type='number', debounce=True, className="field-body-textbox")
            ], className="field"),
            ...
            html.Div([
                html.Div(
                    ...
                    className="field-label"),
                dcc.Checklist(options=option, id="en_optimize_short_bursts", className="field-body",
            ...
            html.Div([
                html.Div([
                    html.Div(
                        ...
                        className="field-label"),
                    dcc.Input(id='vfo_' + str(i) + '_freq',
                              value=webInterface_inst.module_signal_processor.vfo_freq[i] / 10 ** 6, type='number',
                              debounce=True, className="field-body-textbox")
                ], className="field"),
                ...
                html.Div([
                    html.Div(
                        ...
                        className="field-label"),
                    dcc.Input(id='vfo_' + str(i) + '_bw', value=webInterface_inst.module_signal_processor.vfo_bw[i],
                              type='number', debounce=True, className="field-body-textbox")
                ], className="field"),

                html.Div([
                    html.Div(
                        "VFO-" + str(i) + " Squelch [dB] :",
                        className="field-label"),
                    dcc.Input(id='vfo_' + str(i) + '_squelch',
                              value=webInterface_inst.module_signal_processor.vfo_squelch[
                                  i], type='number',
                              debounce=True, className="field-body-textbox")
                ], className="field"),
            ], id="vfo" + str(i), className="card",
                style={'display': 'block'} if i < webInterface_inst.module_signal_processor.active_vfos else {
                    'display': 'none'})

    system_control_card = \
        html.Div([
            html.Div([html.Div("Open System Control", id="label_en_system_control", className="field-label"),
                      dcc.Checklist(options=option, id="en_system_control", className="field-body",
                                    value=webInterface_inst.en_system_control),
                      ], className="field"),

            html.Div([
                html.Div([html.Button('Restart Software', id='btn-restart_sw', className="btn-restart_sw", n_clicks=0)],
                         className="field"),
                html.Div([html.Button('Restart System', id='btn-restart_system', className="btn-restart_system",
                                      n_clicks=0)], className="field"),
                html.Div([html.Button('Shutdown System', id='btn-shtudown_system', className="btn-shtudown_system",
                                      n_clicks=0)], className="field"),
                html.Div([html.Button('Clear Cache and Restart', id='btn-clear_cache', className="btn-clear_cache",
                                      n_clicks=0)], className="field"),
    ...
    config_page_component_list = [start_stop_card, daq_status_card, daq_config_card, vfo_config_card, dsp_config_card,
                                  display_options_card, station_config_card, recording_config_card, system_control_card]
